...
def get_constcheck_help() -> t.Optional[str]:
...
def generate_contributing() -> None:
    ...
    content = (
        ...
        .read_text(encoding="utf-8")
        .replace("# Contributing to constcheck", "# Contributing")
    )
    (GENERATED / "contributing.md").write_text(content, encoding="utf-8")


@extension
def generate_constcheck_help() -> None:
    """Generate constcheck help documentation."""
    content = get_constcheck_help()
    if content is not None:
        (GENERATED / "constcheck-help.rst").write_text(
            content,
            encoding="utf-8",
        )
...
def setup(app: Sphinx) -> None:
    ...
    app.connect("builder-inited", generate_constcheck_help)
